...
L=500e-6; C=100e-6; R=12; k=8.4; fs = 40e3; Ts=1/fs; Vl=3.8; Vu=8.2; Vref=11; Rg = 10e-3; Ron=15e-3; n = 4.7;
...
Vg=5;incr=0.1; lambda=zeros(2); % initializari parametru de simulare, increment si multiplicatori caracteristici in cerc
 while abs(lambda(1))<1 && abs(lambda(2))<1  
      ...
      X=(I-Phi2*Phi1)\(Phi2*Psi1+Psi2)*Vg;
            
      dFdxn = -k*extr*Phi1;    
      dFddn = -k*extr*Ts*Phi1*(Aon*X+Bon*Vg)-Vu-Vl;
      dgdxn = Phi2*Phi1;
      dgddn = Ts*Phi2*((Aon-Aoff)*Phi1*X-(Phi1*Boff-Aon*Psi1-Bon)*Vg);
